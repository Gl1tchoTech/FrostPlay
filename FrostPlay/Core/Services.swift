...
struct TMDBService: MetadataService {
    ...
    func search(query: String, kind: MediaKind?) async throws -> [MediaItem] {
        ...
        let endpoint = kind == .tv ? "search/tv" : "search/movie"
        ...
        return payload.results.map { result in
            MediaItem(
                id: "tmdb-\(result.id)-\(kind == .tv ? "tv" : "movie")",
...
struct AniListService: MetadataService {
    ...
    func search(query: String, kind: MediaKind?) async throws -> [MediaItem] {
        let request = URLRequestBuilder.postJSON(url: endpoint, body: [
            "query": """
            query ($search: String) { Page(perPage: 20) { media(search: $search, type: ANIME) { id idMal title { romaji english native } description seasonYear coverImage { large } bannerImage episodes } } }
            """,
...
struct VidLinkAdapter: PlaybackSourceAdapter {
    ...
    func playback(for media: MediaItem, season: Int?, episode: Int?, language: String) -> PlaybackFormat? {
        ...
        let path: String
        if media.kind == .movie { path = "movie/\(tmdbID)" }
        else { guard let season, let episode else { return nil }; path = "tv/\(tmdbID)/\(season)/\(episode)" }
...
private struct TMDBResult: Decodable {
    ...
    enum CodingKeys: String, CodingKey { case id, title, name, overview; case posterPath = "poster_path"; case backdropPath = "backdrop_path"; case releaseDate = "release_date"; case firstAirDate = "first_air_date" }
}
private struct AniListResponse: Decodable {
    ...
    struct Anime: Decodable { let id: Int; let idMal: Int?; let title: Title; let description: String?; let seasonYear: Int?; let coverImage: Cover; let bannerImage: String? }
